...
# Uitvoeren simulatie
De simulatie kan uitgevoerd worden door 'runSpuis.py' uit te voeren.
Dit is een Python code die de postprocessing bevat.
Deze voert de executable uit en gebruikt de resultaten om figuren te maken.
Deze figuren worden opgeslagen in de folder waar de input file is gemaakt.
De input file eindigt op .in.
Voorbeelden zijn opgeslagen in folder 'Tests'.



# Installeren requirements Python packages
Om de juiste packages te hebben voor Python om de som te draaien, vul de volgende commando in in de command window:
pip install -r <pad naar>/requirements.txt